...
export default function ScatterPlot(props: ScatterPlotProps) {
  ...
  Object.keys(sopData).map((key: any) => {
        ...
        lean.push(sopData[key][SOP_DATA_COLUMN_LEAN]);
        targetTime.push(sopData[key][SOP_DATA_COLUMN_TARGET]);
      });
  ...
  let generateOneCycleData = (data: any): { [name: string]: Array<any> } => {
    ...
    Object.keys(sopData).map((key: any) => {
      ...
      times.push(sopData[key][SOP_DATA_COLUMN_EVENT_TS]);
      values.push(sopData[key][SOP_DATA_COLUMN_VALUE]);
      names.push(sopData[key][SOP_DATA_COLUMN_NAME]);
      sequence.push(sopData[key][SOP_DATA_COLUMN_SEQUENCE]);
      lean.push(sopData[key][SOP_DATA_COLUMN_LEAN]);
      target.push(sopData[key][SOP_DATA_COLUMN_TARGET]);
    });

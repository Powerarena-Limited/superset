...
import ExploreChartPanel from './PaExploreChartPanel';
...
function ExploreViewContainer(props) {
  ...
  const [tableFormData, setTableFormData] = useState(tableFormDataObject);
  const [tableQueriesResponse, setTableQueriesResponse] = useState({});
  ...
  const onQuery = useCallback(() => {
    setIsTableUpdate(~isTableUpdate);
    props.actions.triggerQuery(true, props.chart.id);
    addHistory();
    setLastQueriedControls(props.controls);
  }, [props.controls, addHistory, props.actions, props.chart.id]);

  function updateTableFormData(data, chartName) {
    handleTableFormData(data, chartName);
    setIsTableUpdate(~isTableUpdate);
  }
  ...
  function handleTimeRange(value) {
    tableFormData.update_time_range = value;
  }
  ...
  // Effect when raw data need changed
  useEffect(() => {
    if (tableFormData.update_time_range) {
  ...
  function renderChartContainer() {
    return (
      <ExploreChartPanel
        ...
        tableQueriesResponse={tableQueriesResponse}
        updateTableForm={updateTableFormData}
